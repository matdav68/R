...
```{r}
conf_summary_inter <- conf_summary %>%
  filter(conf1 != conf2)
```
...
```{r}
conf_heatmap <- conf_summary_inter %>%
...
conf_results <- wc_inter %>%
  ...
  count(home_conf, away_conf, winner_conf)
```
...
# Use win rate and goals per match as features
conf_features <- conf_table %>%
  select(conf, win_rate, goals_per_game, gd) %>%
  column_to_rownames("conf")

# Compute distance and hierarchical clustering
dist_mat <- dist(conf_features)
hc <- hclust(dist_mat, method = "ward.D2")

# Plot dendrogram
plot(hc, main = "Confederation Clustering by Performance", ylab = "Distance")
```
