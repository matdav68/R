...
```{r}
conf_metrics <- conf_table %>%
  select(conf, win_rate, goals_per_game) %>%
  column_to_rownames("conf")
conf_scaled <- scale(conf_metrics)

set.seed(123)  # for reproducibility
km_model <- kmeans(conf_scaled, centers = 3, nstart = 25)

# Add cluster assignment to your table
conf_clustered <- conf_table %>%
  mutate(cluster = km_model$cluster)

ggplot(conf_clustered, aes(x = goals_per_game, y = win_rate, label = conf, color = factor(cluster))) +
  geom_point(size = 5, alpha = 0.8) +
  geom_text(vjust = -1, size = 4.2) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  ...
  labs(
    ...
    subtitle = "Based on Win Rate and Goals Per Game",
    x = "Goals Per Game",
    y = "Win Rate"
  ) +
  theme_minimal(base_size = 14)
